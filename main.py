...
from calibre_plugins.horver.resources.opencc_python.opencc import OpenCC
...
class TradSimpChinese(Tool):
    ...
    def create_action(self, for_toolbar=True):
        ...
        # the plugins menu
        ac = QAction(get_icons('images/icon.png'), _('Convert content between horizontal and vertical'), self.gui)
        if not for_toolbar:
            # Register a keyboard shortcut for this toolbar action. We only
            # register it for the action created for the menu, not the toolbar,
            # to avoid a double trigger
            self.register_shortcut(ac, 'HorVer convert', default_keys=('Ctrl+Shift+H',))
    ...
    def process_files(self, criteria):
        ...
        if criteria[0]:
            ...
            from calibre_plugins.horver.resources.dialogs import ShowProgressDialog
    ...
    def convert_text(self, data, criteria):
        from calibre_plugins.horver.resources.utilities import tokenize
